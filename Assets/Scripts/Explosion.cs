...
public class Explosion: WarEntity
{
    ...
    public void Initialise(Vector3 position, float blastRadius, float damage = 0f)
    {
        if (damage > 0f)
        {
            TargetPoint.FiilBuffer(position, blastRadius);
            for (int i = 0; i < TargetPoint.BufferedCount; i++)
            {
                TargetPoint.GetBuffer(i).Enemy.TakeDamage(damage);
            }
        }
